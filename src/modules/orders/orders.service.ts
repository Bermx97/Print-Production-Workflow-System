...
export const createStepEventV2 = async (orderNumber: number, userId: string, role: employee_role, eventType: EventType, stepQuantity?: number) => {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (eventType === 'END') {
      ...
      return handleEnd({tx, order, userId, role, state, wf, stepQuantity });
    }
  });
};